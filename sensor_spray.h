...
void spray (bool array[3], bool pause)
{
if (pause) {
    if (millis()-Time==3000) {
      pause=false;
  ...
  if ((array[0] && array[1] && array[2])==1)     //Si une mauvaise herbe a été détecté et que les niveaux de batterie et de désherbant sont suffisants 
 {
